...
class InternalApplicant extends Model
{
    use HasFactory;
    use SoftDeletes;
    use LogsActivity;
    ...
    public function getPdsAttribute()
    {
        return $this->getMedia('internal_applicant_pds')->map(function ($item) {
            $media = $item->toArray();
    ...
    public function getWesAttribute()
    {
        return $this->getMedia('internal_applicant_wes')->map(function ($item) {
            $media = $item->toArray();
            $media['url'] = $item->getUrl();
